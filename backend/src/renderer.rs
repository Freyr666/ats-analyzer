...
impl Renderer<VideoR> {
    pub fn new (port: i32, bin: gst::Bin) -> Renderer<VideoR> {
        let mut is_vaapi = true;
        let encoder = match gst::ElementFactory::make("vaapivp8enc", None) {
            Some (e) => e,
            None     => {
                is_vaapi = false;
                gst::ElementFactory::make("vp8enc", None).unwrap()
            },
        };
        ...
        if is_vaapi {
            encoder.set_property("rate-control", &enum_to_val("GstVaapiRateControlVP8", 2)).unwrap();

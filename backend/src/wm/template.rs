...
pub struct ContainerTemplate {
    ...
    pub widgets:  HashMap<String,WidgetDesc>,
}

#[derive(Serialize,Deserialize,Clone)]
pub struct WmTemplatePartial {
    resolution: (u32, u32),
    layout:     HashMap<String,ContainerTemplate>,
}

#[derive(Serialize,Deserialize,Clone)]
pub struct WmTemplate {
    pub resolution: (u32, u32),
    pub layout:     HashMap<String,ContainerTemplate>,
    pub widgets:    HashMap<String,WidgetDesc>,
}
    ...
    pub fn validate (&self) -> Result<(),String> {
        ...
        for (ref cname, ref c) in &self.layout {
            ...
            for (ref wname, ref w) in &c.widgets {
                if ! self.widgets.iter().any(|(name,_)| name == *wname) {
                ...
                // not very optimal
                if c.widgets.iter().any(|(name,wdg)| name != *wname && wdg.position.is_overlap(&w.position)) {

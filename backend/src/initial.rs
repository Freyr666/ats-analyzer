...
pub fn uri_error_to_string(e : &UriError) -> String {
    match *e {
        ...
        UriError::OctetValue (v,o)      => format!("bad value ({}) in ip octet = {}", v, o + 1),
        UriError::OctetMissing(o)       => format!("ip octet {} is missing", o + 1),
        UriError::OctetNumber (o)       => format!("bad number ({}) of ip octets", o + 1),
        UriError::BadSymbol(s)          => format!("bad symbol: {}",s),
        UriError::PortValue(v)          => format!("bad port value ({})",v),
...
pub fn validate_uri(uri : &String) -> Result<&String, UriError> {

    let prefix = "udp://";
    if !uri.starts_with(prefix) {
        return Err (UriError::Prefix)
    }
    else {
